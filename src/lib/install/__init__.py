...
class Install(ctk.CTk):
	def __init__(self):
		...
		self.url_title = ctk.CTkLabel(self.container_url, text='Caminho de instalacao: ')
		...
		self.log_text.insert('end', 'Logs de instalacao aparecerao aqui.\n')
	...
	def install(self):
		...
		self.progress_counter.configure(text='Preparando instalacao...')
		self.progress_bar.configure(mode='indeterminate')
		...
		try:
			...
			if fallback.resolve() == destination.resolve():
				raise PermissionError(
					'Sem permissao para instalar na pasta configurada. Execute o instalador com privilegios elevados.'
				)
	...
	def _migrate_legacy_installation(self, destination):
		...
		if migrated_files:
			self.after(
				0,
				self._append_log,
				'Arquivos migrados para dados do usuario: ' + ', '.join(Path(path).name for path in migrated_files),
			)
		if skipped_files:
			self.after(
				0,
				self._append_log,
				'Arquivos de dados ja existentes foram preservados: '
		...
		for legacy_path in self._legacy_installation_paths():
			...
			self.after(0, self._append_log, f'Removendo instalacao antiga em: {legacy_path}')
	...
	def _create_start_menu_shortcuts(self, destination):
		...
		if not main_exe.exists():
			raise FileNotFoundError(f'Executavel principal nao encontrado para atalho: {main_exe}')

		shortcut_definitions = [('Passwords Manager.lnk', main_exe, 'Passwords Manager')]
		...
		for start_menu_dir in windows_start_menu_directories():
			...
			for shortcut_name, target_exe, description in shortcut_definitions:
				shortcut_path = start_menu_dir / shortcut_name
				try:
					create_windows_shortcut(shortcut_path, target_exe, destination_path, description)
					created_paths.append(str(shortcut_path))
				except Exception as shortcut_error:
					failures.append(f'{shortcut_path}: {shortcut_error}')

		if not created_paths and failures:
			raise RuntimeError(
				'Falha ao criar atalho no Menu Iniciar: ' + '; '.join(failures[:3])
			)

		return created_paths

	def _create_desktop_shortcuts(self, destination):
		if os.name != 'nt':
			return []
		...
		for desktop_dir in windows_desktop_directories():
			...
			shortcut_path = desktop_dir / 'Passwords Manager.lnk'
			try:
				create_windows_shortcut(
					shortcut_path,
					main_exe,
		...
		if not created_paths and failures:
			raise RuntimeError('Falha ao criar atalho na Area de Trabalho: ' + '; '.join(failures[:3]))
	...
	def _on_install_success(self, destination, is_update, updated_files, skipped_files, shortcuts=None):
		...
		self.progress_counter.configure(text='Atualizacao concluida' if is_update else 'Instalacao concluida')
		self._append_log(
			f'Processo concluido com sucesso. Copiados: {updated_files}. Inalterados: {skipped_files}.'
		)
		if shortcuts:
	...
	def _on_install_error(self, error_message):
		...
		self.progress_bar.configure(mode='determinate')
		self.progress_bar.set(0)
		self.progress_counter.configure(text='Falha na instalacao')
		self._append_log(f'Erro: {error_message}')
		messagebox.showerror(program_name, f'Falha na instalacao:\n{error_message}')

	def _install_worker(self, destination, is_update):
		try:
			self.after(0, self._append_log, 'Procurando arquivos de deploy pre-compilados...')
			try:
				release_dir, files = self._release_files()
				self.after(0, self._append_log, f'Payload encontrado em: {release_dir}')
			except FileNotFoundError:
				self.after(0, self._append_log, 'Payload nao encontrado. Iniciando build local...')
				self._run_build_script()
				self.after(0, self._append_log, 'Build concluido. Preparando copia...')
				release_dir, files = self._release_files()
			...
			if used_fallback:
				self.after(
					...
					self._append_log,
					(
						'Sem permissao no destino selecionado. '
						f'Usando fallback automatico: {destination}'
					),
				)

			self.after(0, self._append_log, f'Dados do usuario serao armazenados em: {local_data_path()}')
			...
			if os.name == 'nt':
				try:
					...
					self.after(0, self._append_log, f'Registro de desinstalacao criado: {reg_path}')
			...
			if used_fallback:
				self.after(
					...
					program_name,
					(
						'A pasta selecionada exigia permissao elevada.\n'
	...
	def save_logs(self):
		...
		file_path = filedialog.asksaveasfilename(
			title='Salvar logs da instalacao',
		...
		try:
			...
			messagebox.showerror(program_name, f'Nao foi possivel salvar os logs:\n{exception}')
			return
	...
	def open_install_folder(self):
		...
		if not destination.exists():
			messagebox.showwarning(program_name, 'A pasta de instalacao nao foi encontrada.')
			return
